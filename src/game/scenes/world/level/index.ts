...
import type {
  ...
  import twMap from './realmaps/tw2.json';
  ...
  export class Level extends TileMatrix implements ILevel {
    ...
    private getTerrainAt(x: number, y: number) {
      if (y >= 0 && y < this.terrainMap.length && x >= 0 && x < this.terrainMap[y].length) {
    ...
    if (newScenario) {
      ...
      const terrainLayer = this.mapData.layers.find((l: any) => l.name === "tiles");
      
      for (let y = 0; y < this.mapHeight; y++) {
        for (let x = 0; x < this.mapWidth; x++) {
          const index = y * this.mapWidth + x;
          const tileId = terrainLayer.data[index];
          
          // Map Tiled tile ID to your game's terrain types
          switch (tileId) {
            case 1: // Deepwater
            this.terrainMap[y][x] = TerrainType.DEEPWATER;
            ...
            break;
            default:
            //this.terrainMap[y][x] = TerrainType.UNKNOWN;
            break;
          }
        }
      }
